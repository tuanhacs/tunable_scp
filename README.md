...
Adaptive miscoverage levels are selected over the full interval
`[method.epsilon, 1 - method.epsilon]` (default `epsilon: 1.0e-10`). TsCP
uses the exact empirical-quantile rank breakpoints; eCP solves the size
condition directly in regression and uses e-value inclusion breakpoints in
classification. LOO estimators follow the same rules after deleting each
observation. There is no fixed alpha grid. If the budget cannot be met anywhere
in the allowed interval, the run raises a `ValueError` instead of silently
returning a prediction set that violates the requested size. Results from
earlier fixed-grid runs are not directly comparable and should be regenerated.
...
for the size-control panel. Panel-specific limits override global limits.
